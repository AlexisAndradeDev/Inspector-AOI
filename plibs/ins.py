        ...
        if skip:
            ...
            if skip:
                ...
                if settings["check_mode"] != "check:no":
                    cv_functions.export_images(skip_images, photo_number, board.get_number(), settings["skip_function"]["name"], settings["skip_function"]["light"], images_path=general_settings["images_path"])
...
def inspect_photo(photo_number, photo, inspection_points, registration_settings, settings, photo_ultraviolet=None):
    ...
    if settings["boards_num"] == 1:
        board = cv_functions.ObjectInspected(board_number=1)
        start = timer()
